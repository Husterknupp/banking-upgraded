...
app.get("/whatElseIsComing", (req, res) => {
  const { documentUrl, queriedDay } = req.query;

  getDocument(documentUrl).then(({ monthly, yearly }) => {
    const firstDayOfMonth = queriedDay || 1;
    const today = new Date();
    const currentMonth = getMonthOfDate(today) + 1;
    const daysOFCurrentMonth =
      queriedDay || new Date(fullYear(today), currentMonth, 0).getDate();
...
function getDocument(url) {
  return axios
    .get(new URL(url).toLocaleString())
    .then(({ data }) => {
